...
$http.get('http://'+ dir +':8080/api/subject').success(function(data) {
        $scope.subjects = data;
    })
    .error(function(data) {
...
$scope.GetAll = function() {
    ...
    $scope.filterSubject = function(res) {
        ...
        $http.post('http://'+ dir +':8080/api/sub', $scope.newSubject)
            .success(function (data) {
                if (data==false) {
                    alert("No hay dicha asignatura");
                }
                else {
                    $scope.subjects = data;
    ...
    $scope.filterName = function(res) {
        ...
        $http.post('http://'+ dir +':8080/api/nam', $scope.newSubject)
            .success(function (data) {
                if (data==false) {
...
$scope.registrarSubject = function(res) {
        ...
        $http.post('http://'+ dir +':8080/api/subject', $scope.newSubject)
            .success(function (data) {
...
$scope.modificarSubject = function(newSubject) {
    $http.put('http://'+ dir +':8080/api/subject/' + $scope.newSubject._id, $scope.newSubject)
        .success(function(data) {
            $scope.cleanall(); // Borramos los datos del formulario
            $scope.subjects = data;
            $scope.selected = false;
...
$scope.borrarSubject = function(newSubject) {
    $http.delete('http://'+ dir +':8080/api/subject/' + $scope.newSubject._id)
        .success(function(data) {
            $scope.cleanall();
            $scope.subjects = data;
            $scope.selected = false;
